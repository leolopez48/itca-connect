  ...
  methods: {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        try {
          const { data } = await roleApi.put(`/${edited.id}`, edited);
      ...
      //Creating record
      try {
        const { data } = await roleApi.post(null, this.editedItem);
    ...
    async deleteItemConfirm() {
      try {
        const { data } = await roleApi.delete(`/${this.editedItem.id}`, {
          params: {
    ...
    getDataFromApi(options) {
      ...
      this.debounce = setTimeout(async () => {
        try {
          const { data } = await roleApi.get(null, {
