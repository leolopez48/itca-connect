  ...
  methods: {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        try {
          const { data } = await campusApi.put(`/${edited.id}`, edited);
      ...
      //Creating record
      try {
        const { data } = await campusApi.post(null, this.editedItem);
    ...
    async deleteItemConfirm() {
      try {
        const { data } = await campusApi.delete(`/${this.editedItem.id}`, {
          params: {
    ...
    getDataFromApi(options) {
      ...
      this.debounce = setTimeout(async () => {
        try {
          const { data } = await campusApi.get(null, {
