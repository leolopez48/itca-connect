...
import detailCampusPlaceApi from "@/services/detailCampusPlaceApi";
...
name: {
  ...
  methods: {
    async initialize() {
      ...
      if (responses) {
        this.campus = responses[1].data.campus;
this.placetype = responses[2].data.placetype;
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        try {
          const { data } = await detailCampusPlaceApi.put(`/${edited.id}`, edited);
      ...
      //Creating record
      try {
        const { data } = await detailCampusPlaceApi.post(null, this.editedItem);
    ...
    async deleteItemConfirm() {
      try {
        const { data } = await detailCampusPlaceApi.delete(`/${this.editedItem.id}`, {
          params: {
    ...
    getDataFromApi(options) {
      ...
      this.debounce = setTimeout(async () => {
        try {
          const { data } = await detailCampusPlaceApi.get(null, {
