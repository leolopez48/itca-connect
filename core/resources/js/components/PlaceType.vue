      ...
      <v-card>
        ...
        <!-- icon -->
            <v-col cols="12" sm="12" md="4">
                <base-input
                label="Icon"
                v-model="v$.editedItem.icon.$model"
                :rules="v$.editedItem.icon"
                />
            </v-col>
        <!-- icon -->
  ...
  methods: {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        try {
          const { data } = await placeTypeApi.put(`/${edited.id}`, edited);
      ...
      //Creating record
      try {
        const { data } = await placeTypeApi.post(null, this.editedItem);
    ...
    async deleteItemConfirm() {
      try {
        const { data } = await placeTypeApi.delete(`/${this.editedItem.id}`, {
          params: {
    ...
    getDataFromApi(options) {
      ...
      this.debounce = setTimeout(async () => {
        try {
          const { data } = await placeTypeApi.get(null, {
